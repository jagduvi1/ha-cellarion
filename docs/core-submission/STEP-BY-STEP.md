...
A walkthrough for opening the three pull requests yourself. Order matters:
brands and documentation first, because the core pull request links to
both. Budget about an hour for the three, plus whatever you spend writing.
...
3. Open a pull request from your `cellarion` branch to `home-assistant/brands`
   `master`. Their template asks for the integration domain (`cellarion`),
   whether it is a core integration (yes, pull request pending; add the link
   once it exists), and confirmation the images follow the rules. Facts:
   icon 256×256 and 512×512, logo 322×256 and 488×388, all PNG, transparent,
   trimmed.
4. Note the pull request URL; the core pull request needs it.
...
4. Open a pull request against `home-assistant/home-assistant.io` **`next`**
   ...
   for a link to the core pull request; add that link after step 3 by
...
2. The template loads automatically. Fill it in completely and keep every
   ...
   - *Additional information*: the documentation and brands pull request
     links.
...
4. Go back to the docs and brands pull requests and paste the core pull
   request link into their descriptions.
